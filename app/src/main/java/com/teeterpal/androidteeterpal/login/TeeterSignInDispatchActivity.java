 ...
 */
public abstract class TeeterSignInDispatchActivity extends Activity {
    protected abstract Class<?> getTargetClass();

    private static final int LOGIN_REQUEST = 0;
    private static final int TARGET_REQUEST = 1;

    private static final String LOG_TAG = "TeeterLoginDispatch";
    ...
    protected Intent getParseLoginIntent() {
        Intent intent = new Intent(this, TeeterSignInSignUpActivity.class);
        return intent;
    }

    private void runDispatch() {
        if (ParseUser.getCurrentUser() != null) {
            debugLog(getString(R.string.teeter_signin_dispatch_user_signed_in) + getTargetClass());
            startActivityForResult(new Intent(this, getTargetClass()), TARGET_REQUEST);
        } else {
            debugLog(getString(R.string.teeter_signin_dispatch_user_not_signed_in));
